getTableAfterPCA = function(datSpecies,analysisName="",pcaYesNo="pca",criterion=70){
  ...
  if(pcaYesNo=="pca"){
    ...
    if(is.numeric(criterion)){
      nbaxes=which(log.pca$eig[,3]>criterion)[1]   # we are taking the axis until having X% of total inertia
    ...
    par(mar=c(4,4,1,1))
    color=rep("grey",length(log.pca$eig[,1]))
    if(is.numeric(criterion)) color[1:nbaxes]="green"
