...
## Figures for Metier Analysis for West Coast Groundfish
...
#Proportion of a species' EVR in each cluster/metier
summarizeddata = casting %>%
  group_by(clusternumber) %>% 
  summarise(across("ARROWTOOTH FLOUNDER":"YELLOWTAIL ROCKFISH", 
                   sum, na.rm = TRUE)) 

summarizeddata=sweep(summarizeddata[, -1], 2, colSums(summarizeddata[, -1]), FUN = "/")

summarizeddata$clusternumber = 1:17

summarizeddata = relocate(summarizeddata, clusternumber)

clustersummary=summarizeddata %>%
  pivot_longer(cols = -clusternumber, names_to = "Category", values_to = "Value")

plot1 = ggplot(clustersummary, aes(x = clusternumber, y = Value)) +
...
clustersummary$SpeciesNumber=as.numeric(factor(clustersummary$Category))

plot2 = ggplot(clustersummary, aes(x = SpeciesNumber, y = Value, fill = factor(SpeciesNumber))) +
  geom_bar(stat = "identity", position = "dodge") +
  facet_wrap(~clusternumber, scales = "free_x", ncol = 3) +
  scale_x_continuous(
    breaks = 1:length(unique(clustersummary$Category)),  
    labels = 1:length(unique(clustersummary$Category))) +
  scale_fill_manual(
    values = colors_31, 
    labels = paste(1:length(unique(clustersummary$Category)), unique(clustersummary$Category), sep = ": ")) +
...
#Proportion of a species' EVR in a metier

metiersummarydata=metiercasting %>%
  group_by(metier) %>% 
  summarise(across("ARROWTOOTH FLOUNDER":"YELLOWTAIL ROCKFISH", 
                   sum, na.rm = TRUE)) 

metiersummarydata=sweep(metiersummarydata[, -1], 2, colSums(metiersummarydata[, -1]), FUN = "/")

metiersummarydata$metiers = c("A", "B", "C", "D")

metiersummarydata = relocate(metiersummarydata, metiers)

metiersummary=metiersummarydata %>%
  pivot_longer(cols = -metiers, names_to = "Category", values_to = "Value")


metiersummary$SpeciesNumber=as.numeric(factor(metiersummary$Category)) 

plot2 = ggplot(metiersummary, aes(x = SpeciesNumber, y = Value, fill = factor(SpeciesNumber))) +
  geom_bar(stat = "identity", position = "dodge") +
  facet_wrap(~metiers, scales = "free_x", ncol = 2) +
  scale_x_continuous(breaks = 1:length(unique(metiersummary$Category)),  
                     labels = 1:length(unique(metiersummary$Category)) ) +
  scale_fill_manual(values = colors_31, 
                    labels = paste(1:length(unique(metiersummary$Category)), unique(metiersummary$Category), sep = ": ")) +
  guides(fill = guide_legend(ncol = 1)) +
  theme_minimal()+
  theme(legend.title = element_text(size = 30),
        legend.text = element_text(size = 15),
...
#Proportion of a cluster or metier's EVR occupied by a particular species
...
stackbar = ggplot(longdata, aes(x = factor(clusternumber), y = proportion, fill = species)) +
...
longdatametier=longdatametier %>%
  group_by(metier) %>%
  mutate(proportion = catch / sum(catch))

stackbartestm = ggplot(longdatametier, aes(x = factor(metier), y = proportion, fill = species)) +
  geom_bar(stat = "identity") +
  labs(x = "Metier",
...
# Proportion of a gear's EVR in a specific metier/cluster
