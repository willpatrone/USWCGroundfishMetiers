...
plot1 = ggplot(tidy_data, aes(x = clusternumber, y = Value)) +
  ...
  theme_minimal() +
  labs(x = "Cluster Number", y = "Proportion of EVR in Each Cluster", title = "Proportion of EVR by Species in Each Cluster") +
  theme( 
...
ggsave("Figure A2 - Proportion of EVR by Species in Each Cluster.jpg", 
       plot = plot1, 
...
plot2 = ggplot(tidy_data, aes(x = SpeciesNumber, y = Value, fill = factor(SpeciesNumber))) +
  ...
  scale_fill_manual(
    values = scales::viridis_pal()(length(unique(tidy_data$Category))), 
  ...
  labs(
    ...
    title = "Proportion of EVR in Each Metier by Species",
    fill = "Species Key" 
  ) 
  
plot2
ggsave("Figure A5 - Proportion of EVR by Species for Each Cluster.jpg",
       plot = plot2, 
       width = 30, 
       height = 20, 
       units = "in", 
...
silhouettescores = c(0, 0, 0, 0.3235494, 0.3908636, 0.3991081, 0.4037481, 
                       ...
                       0.4449974, 0.4475046)
df = data.frame(
  nclusters = 1:length(silhouettescores),
...
silhouetteplot = ggplot(df, aes(x = nclusters, y = silhouette)) +
  ...
  labs(
    ...
    y = "Silhouette score",
    title = "Silhouette of the Classification"
  ) +
  theme_minimal()


ggsave("Figure A1: Silhouette Classification Plot for CLARA Clustering",  
       plot = silhouetteplot, 
       width = 15,  
       height = 10,
       units = "in", 
